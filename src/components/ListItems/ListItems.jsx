...
const ListItems = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const { data } = await axios.get('https://quickbite-backend-6dvr.onrender.com/api/items');
        setItems(data);
      } catch (err) {
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      await axios.delete(`https://quickbite-backend-6dvr.onrender.com/api/items/${itemId}`);
